...
if [ "$1" = 'start-kafka' -a "$(id -u)" = '0' ]; then
    chown -R kafka:kafka /data /logs
    exec su-exec kafka "$0" "$@"
fi

# Adapted from https://github.com/ches/docker-kafka
#
# If a ZooKeeper container is linked with the alias `zookeeper`, use it.
# You MUST set ZOOKEEPER_IP in env otherwise.
[ -n "${ZOOKEEPER_PORT_2181_TCP_ADDR}" ] && export ZOOKEEPER_IP=${ZOOKEEPER_PORT_2181_TCP_ADDR}
[ -n "${ZOOKEEPER_PORT_2181_TCP_PORT}" ] && export ZOOKEEPER_PORT=${ZOOKEEPER_PORT_2181_TCP_PORT}
...
[ -z "${ZOOKEEPER_CONNECTION_STRING}" ] && export ZOOKEEPER_CONNECTION_STRING="${ZOOKEEPER_IP}:${ZOOKEEPER_PORT:-2181}"

SERVER_CONFIG="${KAFKA_HOME}/config/server.properties"
cat ${SERVER_CONFIG}.template | sed \
